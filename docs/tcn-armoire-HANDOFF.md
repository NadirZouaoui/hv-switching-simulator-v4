...
## Fichiers (emplacements dans ce dépôt)
| Fichier | Rôle |
|---|---|
| `public/tcn/index.html` | La fiche. Autonome, ~6 ko, mobile-first, mode sombre inclus. Vite recopie `public/` tel quel dans `dist/`, donc la fiche est publiée à `/tcn/` par le déploiement existant. |
| `public/tcn/qr_fiche.svg` / `.png` | QR pointant vers l'URL de production (ci-dessous). Publiés eux aussi, donc téléchargeables depuis le site. |
| `tools/gen_qr_url.py` | Génère le QR. `python tools/gen_qr_url.py <URL>` (dépend de `segno` ; `matplotlib` seulement pour `draw_qr`, importé à la demande). |

## Déploiement (GitHub Pages)
Le dépôt publie déjà sur Pages via la branche `gh-pages` (`npm run deploy`,
qui enchaîne `vite build` puis `gh-pages -d dist`). La fiche suit le même
chemin — rien à configurer de plus.
...
URL de production :
    ...
    https://nadirzouaoui.github.io/hv-switching-simulator-v4/tcn/

Vérifier que l'URL répond, puis la scanner depuis un vrai téléphone avant
d'imprimer l'étiquette.

Pour régénérer le QR après un changement d'URL :

    python tools/gen_qr_url.py https://nadirzouaoui.github.io/hv-switching-simulator-v4/tcn/

### Taille d'impression
Cette URL sort en **version 6 (41 × 41 modules)**, ECC Q — plus dense que les
29 × 29 visés initialement, parce que le nom du dépôt est long. Conséquence
...
Imprimer à **40 mm de côté minimum** pour retrouver la marge de ~0,8 mm par
module nécessaire sur une porte d'armoire empoussiérée. Zone silencieuse
blanche de 4 modules conservée.

Si 30 mm est imposé par l'étiquette, il faut raccourcir l'URL : un dépôt
dédié `nadirzouaoui.github.io/tcn/` ramènerait le QR en version 3–4.
